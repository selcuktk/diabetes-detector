...
class dlf:
    ...
    def L_model_backward(AL, Y, stores, RMS_grads):
        ...
        # After calculating db and dW for the last layer, SdW and Sdb are calculated for the last layer.
        RMS_grads['SdW' + str(L)] = 0.9*(RMS_grads['SdW' + str(L)]
                                         ) + 0.1*(np.square(grads['dW' + str(L)]))
        RMS_grads['Sdb' + str(L)] = 0.9*(RMS_grads['Sdb' + str(L)]
        ...
        for l in reversed(range(L-1)):
            ...
            # After calculating db and dW for each layer, SdW and Sdb are calculated for each layer.
            RMS_grads['SdW' + str(l+1)] = 0.9*(RMS_grads['SdW' + str(l+1)]
                                               ) + 0.1*(np.square(grads['dW' + str(l+1)]))
            RMS_grads['Sdb' + str(l+1)] = 0.9*(RMS_grads['Sdb' + str(l+1)]
    ...
    def update_parameters(parameters, grads, learning_rate, RMS_grads):
        ...
        for l in range(L):
            parameters["W" + str(l+1)] -= learning_rate * grads["dW" +
                                                                str(l+1)] / (np.sqrt(RMS_grads['SdW' + str(l+1)]) + 10**(-8))
            parameters["b" + str(l+1)] -= learning_rate * grads["db" +
                                                                str(l+1)] / (np.sqrt(RMS_grads['Sdb' + str(l+1)]) + 10**(-8))
        return parameters
    ...
    def L_layer_model(X, Y, layers_dim, learning_rate, num_iterations, print_cost):
        ...
        # For implementation of the RMSprop optimization, RMS_grads are initialized with 0 for every layer of weights
        # Also gradients of RMSprop should be initialized before the loop just like weight and bias parameters of the deep learning model
        RMS_grads = {}
        L = len(layers_dim)
        for l in range(1, L):
            RMS_grads['SdW' + str(l)] = np.zeros((layers_dim[l],
                                                  layers_dim[l-1]), dtype=float)
            RMS_grads['Sdb' +
                      str(l)] = np.zeros((layers_dim[l], 1), dtype=float)
            assert (RMS_grads['SdW' + str(l)].shape ==
        ...
        for i in range(0, num_iterations):
            ...
            parameters = dlf.update_parameters(
                parameters, grads, learning_rate, RMS_grads)
