...
class dlf:
    ...
    def initialize_parameters(layer_dims):
        ...
        for l in range(1, L):
            ...
            parameters['b' +
                       str(l)] = np.zeros((layer_dims[l], 1), dtype=float)
    ...
    def L_model_backward(AL, Y, stores, adam_grads, i):
        ...
        grads["dA" + str(L-1)], grads["dW" + str(L)], grads["db" + str(L)
                                                            ] = dlf.linear_activation_backward(dAL, current_store, "sigmoid")

        # After calculating dW and db, following calculations are done for adam optimization algorithms (output layer)
        # Momentum coefficient was set to 0.9
        # RMSprop coefficient was set to 0.9 as well
        adam_grads["VdW" + str(L)] = 0.9 * (adam_grads["VdW" +
                                                       str(L)]) + 0.1 * (grads["dW" + str(L)])
        adam_grads["SdW" + str(L)] = 0.9 * (adam_grads["SdW" + str(L)]
                                            ) + 0.1 * (np.square(grads["dW" + str(L)]))
        adam_grads["Vdb" + str(L)] = 0.9 * (adam_grads["Vdb" +
                                                       str(L)]) + 0.1 * (grads["db" + str(L)])
        adam_grads["Sdb" + str(L)] = 0.9 * (adam_grads["Sdb" + str(L)]
                                            ) + 0.1 * (np.square(grads["db" + str(L)]))
        ...
        for l in reversed(range(L-1)):
            ...
            dA_prev_temp, dW_temp, db_temp = dlf.linear_activation_backward(
                grads["dA"+str(l+1)], current_store, "relu")
            grads["dA" + str(l)] = dA_prev_temp
            grads["dW" + str(l + 1)] = dW_temp
            grads["db" + str(l + 1)] = db_temp

            # After calculating dW and db, following calculations are done for adam optimization algorithms (each hidden layers)
            adam_grads["VdW" + str(l+1)] = 0.9 * (adam_grads["VdW" +
                                                             str(l+1)]) + 0.1 * (grads["dW" + str(l+1)])
            adam_grads["SdW" + str(l+1)] = 0.9 * (adam_grads["SdW" + str(l+1)]
                                                  ) + 0.1 * (np.square(grads["dW" + str(l+1)]))
            adam_grads["Vdb" + str(l+1)] = 0.9 * (adam_grads["Vdb" +
                                                             str(l+1)]) + 0.1 * (grads["db" + str(l+1)])
            adam_grads["Sdb" + str(l+1)] = 0.9 * (adam_grads["Sdb" + str(l+1)]
                                                  ) + 0.1 * (np.square(grads["db" + str(l+1)]))
    ...
    def update_parameters(parameters, grads, learning_rate, adam_grads):
        ...
        for l in range(L):
            parameters["W" + str(l+1)] -= learning_rate * adam_grads["VdW" +
                                                                     str(l+1)] / (np.sqrt(adam_grads["SdW" + str(l+1)]) + 10**(-8))
            parameters["b" + str(l+1)] -= learning_rate * adam_grads["Vdb" +
                                                                     str(l+1)] / (np.sqrt(adam_grads["Sdb" + str(l+1)]) + 10**(-8))
        return parameters
    ...
    def L_layer_model(X, Y, layers_dim, learning_rate, num_iterations, print_cost):
        ...
        # For implementation of the Adam optimization,
        # Since it uses both gradients of momentum and RMSprop, they are initialized with 0 for every layer of weights
        # Also these gradients should be initialized before the loop just like weight and bias parameters of the deep learning model
        adam_grads = {}
        L = len(layers_dim)
        for l in range(1, L):
            adam_grads['VdW' + str(l)] = np.zeros((layers_dim[l],
                                                   layers_dim[l-1]), dtype=float)
            adam_grads['SdW' + str(l)] = np.zeros((layers_dim[l],
                                                   layers_dim[l-1]), dtype=float)
            adam_grads['Vdb' +
                       str(l)] = np.zeros((layers_dim[l], 1), dtype=float)
            adam_grads['Sdb' +
                       str(l)] = np.zeros((layers_dim[l], 1), dtype=float)

            assert (adam_grads['VdW' + str(l)].shape ==
                    (layers_dim[l], layers_dim[l-1]))
            assert (adam_grads['SdW' + str(l)].shape ==
                    (layers_dim[l], layers_dim[l-1]))
        ...
        for i in range(0, num_iterations):
            ...
            grads, adam_grads = dlf.L_model_backward(
                AL, Y, stores, adam_grads, i)

            parameters = dlf.update_parameters(
                parameters, grads, learning_rate, adam_grads)
